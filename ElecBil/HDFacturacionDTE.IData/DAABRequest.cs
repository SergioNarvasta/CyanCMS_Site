    ...
    public class DAABRequest
    {
        public class Parameter : IDbDataParameter
        {
            ...
            public Parameter(string sParameterName, object oValue)
            {
                ...
                m_oParamValue = CastDbTypeValue(oValue);//PROY-24740
                m_TipoBD = DbType.AnsiString;
                m_Direccion = ParameterDirection.Input;
                m_sColumnaOrigen = "";
                m_VersionDato = DataRowVersion.Current;
            ...
            public object Value
            {
                ...
                {
                    //PROY-24740
                    m_oParamValue = CastDbTypeValue(value);
                }
            }

            //PROY-24740
            public object CastDbTypeValue(object value)
            {
                if (value != DBNull.Value && value != null && string.IsNullOrEmpty(value.ToString())!=true)
                {
                    if (m_TipoBD == DbType.Decimal)
                    {
                        return Convert.ToDecimal(value);
                    }
                    else if (m_TipoBD == DbType.Double)
                    {
                        return Convert.ToDouble(value);
                    }
                    else if (m_TipoBD == DbType.Currency)
                    {
                        return Convert.ToDecimal(value);
                    }
                    else if (m_TipoBD == DbType.Date)
                    {
                        return Convert.ToDateTime(value);
                    }
                    else if (m_TipoBD == DbType.DateTime)
                    {
                        return Convert.ToDateTime(value);
                    }
                    else
